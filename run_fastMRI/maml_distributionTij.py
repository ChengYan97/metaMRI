...
experiment_name = 'maml_distribution_dataset500x3_20000epoch'
...
for out_iter in range(OUTER_EPOCH):    # time step
    ...
    # tasks list
    inner_adapt_dataList = [train_set_kneePD, train_set_kneePDFS, train_set_brainFLAIR]
    outer_update_dataList = [val_set_kneePD, val_set_kneePDFS, val_set_brainFLAIR]

    # sample inner adapt data & outer update data
    inner_adapt_data = random.choice(inner_adapt_dataList)
    outer_update_data = random.choice(outer_update_dataList)

    # validation set dataloader
    val_dataloader = torch.utils.data.DataLoader(dataset = outer_update_data, batch_size = 1, num_workers = 8, 
                    shuffle = False, generator = torch.Generator().manual_seed(1), pin_memory = True)

    # update real model
    model.train()
    meta_validation_loss = 0.0
    ...
    for inner_iter in range(INNER_EPOCH):      
        ...
        # sample k examples
        K_examples_dataloader = MAML_K_sampler(inner_adapt_data, K)
        K_examples_inputs = []
        K_examples_targets = []
        ...
        # adapt loss
        validation_loss = 0.0
        for iter, val_batch in enumerate(val_dataloader): 
            val_inputs = val_batch.image.unsqueeze(1).to(device)
            val_targets = val_batch.target.unsqueeze(1).to(device)
        ...
        print('Adapt Validation loss: ', validation_loss.item()/(iter+1))
        meta_validation_loss += validation_loss

    # update θ through validation tasks' loss
    meta_validation_loss = meta_validation_loss / (len(val_dataloader.dataset) * INNER_EPOCH)
    optimizer.zero_grad()
    validation_loss.backward()
    optimizer.step()

    writer.add_scalar("Meta Validation NMSE", meta_validation_loss.item(), out_iter+1)
