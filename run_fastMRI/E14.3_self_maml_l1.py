...
experiment_name = 'E14.3_maml(1st)_out_k-'+LOSS+'(l1_out-5_in-5)'+DOMAIN+'_T300_300epoch'
...
for iter_ in range(EPOCH):    
    ...
    for iter, batch in enumerate(train_dataloader):
        ...
        for inner_iter in range(Inner_EPOCH):
            ...
            for _ in range(adapt_steps): 
                ...
                adapt_loss = l1_loss(Fimg_forward, scale_input_kspace) / torch.sum(torch.abs(scale_input_kspace))
    ...
    writer.add_scalar("Meta 19-step Adaptation Loss (MAML)", meta_adaptation_loss_19/len(train_dataloader), iter_+1)
