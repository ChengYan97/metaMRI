...
# dataloader
train_dataloader_kneePD = torch.utils.data.DataLoader(dataset = trainset_kneePD, batch_size = 1, num_workers = 8, 
                    shuffle = True, generator = torch.Generator().manual_seed(1), pin_memory = False)
train_dataloader_kneePDFS = torch.utils.data.DataLoader(dataset = trainset_kneePDFS, batch_size = 1, num_workers = 8, 
                    shuffle = True, generator = torch.Generator().manual_seed(1), pin_memory = False)
train_dataloader_brainFLAIR = torch.utils.data.DataLoader(dataset = trainset_brainFLAIR, batch_size = 1, num_workers = 8, 
                    shuffle = True, generator = torch.Generator().manual_seed(1), pin_memory = False)

validation_dataloader_kneePD = torch.utils.data.DataLoader(dataset = val_set_kneePD, batch_size = 1, num_workers = 8, 
                    shuffle = False, generator = torch.Generator().manual_seed(1), pin_memory = False)
validation_dataloader_kneePDFS = torch.utils.data.DataLoader(dataset = val_set_kneePDFS, batch_size = 1, num_workers = 8, 
                    shuffle = False, generator = torch.Generator().manual_seed(1), pin_memory = False)
validation_dataloader_brainFLAIR = torch.utils.data.DataLoader(dataset = val_set_brainFLAIR, batch_size = 1, num_workers = 8, 
                    shuffle = False, generator = torch.Generator().manual_seed(1), pin_memory = False)
...
for out_iter in range(OUTER_EPOCH):    # time step
    ...
    validation_loss = 0.0
    validation_loss = evaluate(learner, validation_dataloader, validation_loss, validation_loss_history)
