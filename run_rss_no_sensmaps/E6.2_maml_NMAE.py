...
########################### experiment name ###########################
experiment_name = 'E6.2_maml(NMAE)_T2x300_200epoch'
...
def tuning(model, dataloader, epoch): 
    ...
    for iteration in range(epoch):
        ...
        for iter, batch in enumerate(dataloader): 
            ...
            loss = l1_loss(output, targets) / torch.sum(torch.abs(targets))
...
for iter_ in range(EPOCH):    
    ...
    for index in tqdm(range(0, len(sample_list), Inner_EPOCH)):   
        ...
        for inner_iter in range(Inner_EPOCH):
            ...
            for _ in range(adapt_steps): 
                ...
                adapt_loss = l1_loss(K_examples_preds, K_examples_targets) / torch.sum(torch.abs(K_examples_targets))
            ...
            update_loss = l1_loss(update_output, update_targets) / torch.sum(torch.abs(update_targets))
    ...
    print("Meta Training NMAE (MAML)", meta_training_loss/len(sample_list))
    writer.add_scalar("Meta Training NMAE (MAML)", meta_training_loss/len(sample_list), iter_+1)
    ...
    writer.add_scalar("Validation NMAE", validation_loss, iter_+1)  
