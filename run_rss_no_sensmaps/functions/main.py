...
# An object of this class takes a slice of kspace, the corresponding target image and a mask function
# and outputs the coarse reconstruction image that acts as input to the network and the target image
# normalized to each other. 
# The seed for the mask function is computed from the filename if use_seed is True.
...
def main_train(hp_exp):
    ...
    trainset =  SliceDataset(
            ...
            #path_to_max_vals=hp_exp['path_to_max_vals'],
            #use_SENSE_targets=hp_exp['use_SENSE_targets'],
    ...
    train_loader = torch.utils.data.DataLoader(
            ...
            #collate_fn=my_collate, # use custom collate function here to allow varying input size within a batch
    ...
    #else:
    #    train_meters['train_L2_kspace'] = AverageMeter()

    valid_meters = {'val_SSIM' : AverageMeter(), 'val_PSNR' : AverageMeter(), 'val_L1' : AverageMeter(), 'val_L2' : AverageMeter(), 'val_L2_kspace': AverageMeter()}
    ...
    for epoch in range(save_checkpoint.start_epoch, hp_exp['num_epochs']):
        ...
        for batch_id, batch in enumerate(train_bar):
            ...
            #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).cpu(),'target_kspace',hp_exp,save=save_train_figures)
            #save_figure(input_image[0,0,:,:].cpu(),'input_image',hp_exp,save=save_train_figures)
            #save_figure(target_image[0,0,:,:].cpu(),'target_image',hp_exp,save=save_train_figures)
            #save_figure(ground_truth_image[0,0,:,:].cpu(),'ground_truth_image',hp_exp,save=save_train_figures)
            #save_figure(binary_background_mask[0,0,:,:].cpu(),'binary_background_mask',hp_exp,save=save_train_figures)
            #save_figure(sens_maps[0,0,:,:,0].cpu(),'sens_maps',hp_exp,save=save_train_figures)

            if hp_exp['crop_train_inputs_in_imagedomain']:
            ...
            #save_figure(binary_background_mask[0,0,:,:].cpu(),'binary_background_mask_2',hp_exp,save=save_train_figures)
            #save_figure(output[0,0,:,:].detach().cpu(),'output',hp_exp,save=save_train_figures)
            ...
            if hp_exp['selfsup'] or hp_exp['compute_sup_loss_in_kspace']:
                if hp_exp['selfsup'] and hp_exp['compute_selfsup_loss_in_image_domain']:
                    sens_maps_conj = complex_conj(sens_maps)
                    if hp_exp['use_weighted_target_mask']:
                        target_image_coarse = ifft2c(target_kspace* target_mask_weighted + 0.0) 
                    else:
                        target_image_coarse = ifft2c(target_kspace) 
                    target_image_coarse = complex_mul(target_image_coarse, sens_maps_conj)
                    target_image_coarse = target_image_coarse.sum(dim=1, keepdim=False)
                    target_image_coarse = torch.moveaxis(target_image_coarse , -1, 1)
                    #save_figure(target_image_coarse[0,0,:,:].cpu(),'target_image_coarse',hp_exp,save=save_train_figures)
                    target_train_loss = target_image_coarse
                    ...
                    #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace',hp_exp,save=save_train_figures)
                    
                    # if we do self-supervised training and not just evaluate supervised training in the kspace, then apply mask before loss
                    #if hp_exp['selfsup']:
                    if hp_exp['use_weighted_target_mask']:
                        if hp_exp['compute_train_kspace_loss_not_on_input_lines']:
                            output_kspace = output_kspace * target_mask_weighted * (1-input_mask) + 0.0
                            target_kspace = target_kspace * target_mask_weighted * (1-input_mask) + 0.0
                            #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked_weighted',hp_exp,hp_exp,save=save_train_figures)
                            #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'target_kspace_masked_weighted',hp_exp,hp_exp,save=save_train_figures)
                        else:
                        ...
                        if hp_exp['compute_train_kspace_loss_not_on_input_lines']:
                            output_kspace = output_kspace * target_mask * (1-input_mask) + 0.0
                            target_kspace = target_kspace * target_mask * (1-input_mask) + 0.0
                            #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked',hp_exp,save=save_train_figures)
                            #ave_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'target_kspace_masked',hp_exp,save=save_train_figures)
                        else:
                            output_kspace = output_kspace * target_mask + 0.0
                            #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked',hp_exp,save=save_train_figures)
        ...
        if epoch % hp_exp['val_interval'] == 0:
            ...
            for sample_id, sample in enumerate(valid_bar):
                with torch.no_grad():
                    ...
                    #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).cpu(),'target_kspace_val',hp_exp,save=save_val_figures)
                    #save_figure(input_image[0,0,:,:].cpu(),'input_image_val',hp_exp,save=save_val_figures)
                    #save_figure(target_image[0,0,:,:].cpu(),'target_image_val',hp_exp,save=save_val_figures)
                    #save_figure(ground_truth_image[0,0,:,:].cpu(),'ground_truth_image_val',hp_exp,save=save_val_figures)
                    #save_figure(binary_background_mask[0,0,:,:].cpu(),'binary_background_mask_val',hp_exp,save=save_val_figures)
                    #save_figure(sens_maps[0,0,:,:,0].cpu(),'sens_maps_val',hp_exp,save=save_val_figures)
                    ...
                    #save_figure(output[0,0,:,:].detach().cpu(),'output_val',hp_exp)
                    ...
                    if hp_exp['selfsup'] or hp_exp['compute_sup_loss_in_kspace']:
                        if hp_exp['selfsup'] and hp_exp['compute_selfsup_loss_in_image_domain']:
                            ...
                            target_image_coarse = target_image_coarse.sum(dim=1, keepdim=False)
                            target_image_coarse = torch.moveaxis(target_image_coarse , -1, 1)
                            #save_figure(target_image_coarse[0,0,:,:].cpu(),'target_image_coarse_val',hp_exp,save=save_val_figures)
                            ...
                            # If selfsup computed in imaged domain also val L1 and L2 should be computed w.r.t. coarse target image
                            # and without data consistency
                            # L1 validation loss 
                            loss = val_l1_fct(output, target_image_coarse) / torch.sum(torch.abs(target_image_coarse))
                            valid_meters["val_L1"].update(loss.item())

                            # L2 validation loss 
                            loss = val_mse_reduceSum_fct(output, target_image_coarse) / torch.sum(torch.abs(target_image_coarse)**2)
                            ...
                            # Note that if selfsup=False, then target_mask and target_mask_weighted are just all ones
                            #if hp_exp['selfsup']:
                            if hp_exp['use_weighted_target_mask']:
                                if hp_exp['compute_train_kspace_loss_not_on_input_lines']:
                                    output_kspace = output_kspace * target_mask_weighted * (1-input_mask) + 0.0
                                    target_kspace = target_kspace * target_mask_weighted * (1-input_mask) + 0.0
                                    #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked_weighted_val',hp_exp,save=save_val_figures)
                                    #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'target_kspace_masked_weighted_val',hp_exp,save=save_val_figures)
                                else:
                                    output_kspace = output_kspace * target_mask_weighted + 0.0
                                    target_kspace = target_kspace * target_mask_weighted + 0.0
                                    #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked_weighted_val',hp_exp,save=save_val_figures)
                                ...
                                if hp_exp['compute_train_kspace_loss_not_on_input_lines']:
                                    output_kspace = output_kspace * target_mask * (1-input_mask) + 0.0
                                    target_kspace = target_kspace * target_mask * (1-input_mask) + 0.0
                                    #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked_val',hp_exp,save=save_val_figures)
                                    #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'target_kspace_masked_val',hp_exp,save=save_val_figures)
                                else:
                                    output_kspace = output_kspace * target_mask + 0.0
                                    #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_masked',hp_exp,save=save_val_figures)
                    ...
                    if hp_exp['processing_before_scores_in_image_domain'] == 'binary_masking':
                        output = output * binary_background_mask
                    elif hp_exp['processing_before_scores_in_image_domain'] == 'data_consistency':
                        sens_maps_conj = complex_conj(sens_maps)
                        output_image_data_consistency = output_kspace_fully_sampled*(1-input_mask) + input_kspace
                        #save_figure(torch.log(complex_abs(output_image_data_consistency[0,0,:,:]) + 1e-9).detach().cpu(),'output_kspace_dc_val',hp_exp,save=save_val_figures)
                        output_image_data_consistency = ifft2c(output_image_data_consistency)
                        ...
                        output = torch.moveaxis(output_image_data_consistency , -1, 1)
                    
                    #save_figure(output[0,0,:,:].cpu(),'output_for_SSIM_val',hp_exp,save=save_val_figures)
                    #save_figure(ground_truth_image[0,0,:,:].cpu(),'ground_truth_image_2_val',hp_exp,save=save_val_figures)
                    #save_figure(binary_background_mask[0,0,:,:].cpu(),'binary_background_mask_3_val',hp_exp,save=save_val_figures)
            ...
            val_metric_dict = { #keys should have the same name as the keys used to pick a training loss in hp_exp['loss_functions']
                ...
                #'L1_kspace' : valid_meters['val_L1_kspace'].avg,
            }
            ...
            #if save_checkpoint.start_decay:
            if (val_metric_dict[hp_exp['decay_metric']] < save_checkpoint.best_score and save_checkpoint.mode == "min") or (val_metric_dict[hp_exp['decay_metric']] > save_checkpoint.best_score and save_checkpoint.mode == "max"):
...
def main_test(hp_exp):
    ...
    L2_vals = []
    L2_kspace_vals = []
    L2_kspace_input_positions_vals = [] # only the frequencies given at the input
    L2_kspace_not_input_positions_vals = [] # all the frequencies not given at the input
    L2_kspace_input_positions_center_vals = [] # all the frequencies in the center of the kspace which is always in the input according to our undersampling pattern
    L2_kspace_input_positions_not_center_vals = [] # the remaining frequencies in the input that are not in the center

    MSE_kspace_vals = []
    MSE_kspace_input_positions_vals = [] # only the frequencies given at the input
    MSE_kspace_not_input_positions_vals = [] # all the frequencies not given at the input
    MSE_kspace_input_positions_center_vals = [] # all the frequencies in the center of the kspace which is always in the input according to our undersampling pattern
    MSE_kspace_input_positions_not_center_vals = [] # the remaining frequencies in the input that are not in the center

    # Always compute both, scores after binary masking and after data consistency.
    if hp_exp['processing_before_scores_in_image_domain'] == 'data_consistency':
        ssim_vals_dc = []
        L1_vals_dc = []
        psnr_vals_dc = []
        L2_vals_dc = []
    ...
    for sample_id, sample in enumerate(test_bar):
        with torch.no_grad():
            ...
            #####################
            # new module to remove background artifacts from regions in the image that are not
            ...
            if hp_exp['processing_before_scores_in_image_domain'] == 'binary_masking':
                ...
                sens_maps_conj = complex_conj(sens_maps)
                output_dc = torch.moveaxis(output.clone() , 1, -1 )
                output_per_coil_imgs = complex_mul(output_dc, sens_maps)

                # Transform coil images to kspace
                output_kspace = fft2c(output_per_coil_imgs)

                ################
                # Get L2/MSE scores on differenc partitions in kspace

                # The whole kspace loss
                loss = test_mse_fct_sum(output_kspace, target_kspace) / torch.sum(torch.abs(target_kspace)**2)
                L2_kspace_vals.append(loss.item())
                loss = test_mse_fct_mean(output_kspace, target_kspace)
                MSE_kspace_vals.append(loss.item())

                # Compute different partitions of the output and target kspace
                center_mask = np.zeros(target_kspace.shape[-2])
                size_center = int(round(target_kspace.shape[-2]*hp_exp['center_fraction']))
                pad = (target_kspace.shape[-2] - size_center + 1) // 2
                center_mask[pad : pad + size_center] = 1.0
                center_mask = torch.Tensor(center_mask).to(device)
                center_mask=center_mask.unsqueeze(0).unsqueeze(0).unsqueeze(0).unsqueeze(-1)

                output_kspace_at_input_positions = output_kspace*input_mask + 0.0
                output_kspace_not_at_input_positions = output_kspace*(1-input_mask) + 0.0
                output_kspace_at_input_center_positions = output_kspace*center_mask + 0.0
                output_kspace_at_input_none_center_positions = output_kspace*input_mask*(1-center_mask) + 0.0

                target_kspace_not_at_input_positions = target_kspace*(1-input_mask) + 0.0
                target_kspace_at_input_center_positions = target_kspace*center_mask + 0.0
                target_kspace_at_input_none_center_positions = target_kspace*input_mask*(1-center_mask) + 0.0

                # kspace loss at center positions
                loss = test_mse_fct_sum(output_kspace_at_input_center_positions, target_kspace_at_input_center_positions) / torch.sum(torch.abs(target_kspace_at_input_center_positions)**2)
                L2_kspace_input_positions_center_vals.append(loss.item())
                loss = test_mse_fct_mean(output_kspace_at_input_center_positions, target_kspace_at_input_center_positions)
                MSE_kspace_input_positions_center_vals.append(loss.item())

                # kspace loss at all input positions but the center
                loss = test_mse_fct_sum(output_kspace_at_input_none_center_positions, target_kspace_at_input_none_center_positions) / torch.sum(torch.abs(target_kspace_at_input_none_center_positions)**2)
                L2_kspace_input_positions_not_center_vals.append(loss.item())
                loss = test_mse_fct_mean(output_kspace_at_input_none_center_positions, target_kspace_at_input_none_center_positions)
                MSE_kspace_input_positions_not_center_vals.append(loss.item())

                # kspace loss at all input position
                loss = test_mse_fct_sum(output_kspace_at_input_positions, input_kspace) / torch.sum(torch.abs(input_kspace)**2)
                L2_kspace_input_positions_vals.append(loss.item())
                loss = test_mse_fct_mean(output_kspace_at_input_positions, input_kspace)
                MSE_kspace_input_positions_vals.append(loss.item())

                # kspace loss everywhere but the input positions
                loss = test_mse_fct_sum(output_kspace_not_at_input_positions, target_kspace_not_at_input_positions) / torch.sum(torch.abs(target_kspace_not_at_input_positions)**2)
                L2_kspace_not_input_positions_vals.append(loss.item())
                loss = test_mse_fct_mean(output_kspace_not_at_input_positions, target_kspace_not_at_input_positions)
                MSE_kspace_not_input_positions_vals.append(loss.item())

                #save_figure(torch.log(complex_abs(output_kspace[0,0,:,:]) + 1e-9).cpu(),'output_kspace',hp_exp)
                #save_figure(torch.log(complex_abs(output_kspace_at_input_positions[0,0,:,:]) + 1e-9).cpu(),'output_kspace_at_input_positions',hp_exp)
                #save_figure(torch.log(complex_abs(output_kspace_not_at_input_positions[0,0,:,:]) + 1e-9).cpu(),'output_kspace_not_at_input_positions',hp_exp)
                #save_figure(torch.log(complex_abs(target_kspace_not_at_input_positions[0,0,:,:]) + 1e-9).cpu(),'target_kspace_not_at_input_positions',hp_exp)
                #save_figure(torch.log(complex_abs(target_kspace[0,0,:,:]) + 1e-9).cpu(),'target_kspace',hp_exp)
                #save_figure(torch.log(complex_abs(target_kspace_not_at_input_positions[0,0,:,:]) + 1e-9).cpu(),'target_kspace_not_at_input_positions',hp_exp)
                #save_figure(torch.log(complex_abs(target_kspace_at_input_none_center_positions[0,0,:,:]) + 1e-9).cpu(),'target_kspace_at_input_none_center_positions',hp_exp)
                #save_figure(torch.log(complex_abs(target_kspace_at_input_center_positions[0,0,:,:]) + 1e-9).cpu(),'target_kspace_at_input_center_positions',hp_exp)
                #save_figure(torch.log(complex_abs(input_kspace[0,0,:,:]) + 1e-9).cpu(),'input_kspace',hp_exp)

                #output_kspace_after_dc = output_kspace* (1-input_mask) + input_kspace
                #save_figure(torch.log(complex_abs(output_kspace_after_dc[0,0,:,:]) + 1e-9).cpu(),'output_kspace_after_dc',hp_exp)
                ################
                ...
                # by default I apply the binary mask to output and gt such that SSIM and PSNR scores are only computed where the mask is 1.
                #ground_truth_image = ground_truth_image * binary_background_mask
                #output_image_dc = output_image_dc * binary_background_mask
            ...
            if fname[0] in test_log_filenames_list:
                if slice_num.item() == hp_exp['save_test_images'][fname[0]]:
                    if hp_exp['processing_before_scores_in_image_domain'] == 'data_consistency':
                        ...
                        error = error - error.min() 
                        error = error / error.max()
                        image = torch.cat([ground_truth_image, output, error], dim=0)
                        image = torchvision.utils.make_grid(image, nrow=1, normalize=False)
                        figure = get_figure(image.cpu().numpy(),figsize=(4,12),title=f"ssim={ssim_loss.item():.6f}") 
                        if not os.path.isdir(hp_exp['log_path'] + 'test_imgs/'):
                            os.mkdir(hp_exp['log_path'] + 'test_imgs/')
                        plt.savefig(hp_exp['log_path'] + f"test_imgs/{fname[0]}_s{slice_num.item()}.png")
                        plt.close()
    ...
    if hp_exp['processing_before_scores_in_image_domain'] == 'data_consistency':
        test_metric_dict = {
            ...
            'L2_kspace_m' : np.mean(np.array(L2_kspace_vals)),
            'L2_kspace_s' : np.std(np.array(L2_kspace_vals)),
            'L2_kspace_input_positions_m' : np.mean(np.array(L2_kspace_input_positions_vals)),
            'L2_kspace_input_positions_s' : np.std(np.array(L2_kspace_input_positions_vals)),
            'L2_kspace_not_input_positions_m' : np.mean(np.array(L2_kspace_not_input_positions_vals)),
            'L2_kspace_not_input_positions_s' : np.std(np.array(L2_kspace_not_input_positions_vals)),
            'L2_kspace_input_positions_center_m' : np.mean(np.array(L2_kspace_input_positions_center_vals)),
            'L2_kspace_input_positions_center_s' : np.std(np.array(L2_kspace_input_positions_center_vals)),
            'L2_kspace_input_positions_not_center_m' : np.mean(np.array(L2_kspace_input_positions_not_center_vals)),
            'L2_kspace_input_positions_not_center_s' : np.std(np.array(L2_kspace_input_positions_not_center_vals)),
            'MSE_kspace_m' : np.mean(np.array(MSE_kspace_vals)),
            'MSE_kspace_s' : np.std(np.array(MSE_kspace_vals)),
            'MSE_kspace_input_positions_m' : np.mean(np.array(MSE_kspace_input_positions_vals)),
            'MSE_kspace_input_positions_s' : np.std(np.array(MSE_kspace_input_positions_vals)),
            'MSE_kspace_not_input_positions_m' : np.mean(np.array(MSE_kspace_not_input_positions_vals)),
            'MSE_kspace_not_input_positions_s' : np.std(np.array(MSE_kspace_not_input_positions_vals)),
            'MSE_kspace_input_positions_center_m' : np.mean(np.array(MSE_kspace_input_positions_center_vals)),
            'MSE_kspace_input_positions_center_s' : np.std(np.array(MSE_kspace_input_positions_center_vals)),
            'MSE_kspace_input_positions_not_center_m' : np.mean(np.array(MSE_kspace_input_positions_not_center_vals)),
            'MSE_kspace_input_positions_not_center_s' : np.std(np.array(MSE_kspace_input_positions_not_center_vals)),
        }
